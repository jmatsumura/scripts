...
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "mistral-nemo")  # change to a different model if you need to

def query_ollama(prompt, temperature=0.6, top_k=10, top_p=0.95):
...
def generate_design(past_designs_with_critiques=None):
    ...
    context = (
        "System Prompt:\n"
        "Assume the role of a renowned game designer tasked with creating an innovative 2D action combat game. It must be possible for a solo developer to build it in 3 months while achieving excellence in innovation.\n\n"
        "# Requirements\n"
...
if __name__ == "__main__":
    ...
    filename = f"creator_critic_game_design_mistral-nemo_results_iterations-{args.rounds}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
